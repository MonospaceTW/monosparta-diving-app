...
export default class SpotList extends React.Component {
  ...
  onGetShopList = async () => {
    ...
    const url = `http://9330ffa5.ngrok.io/DivingBackend/public/api/shops/search?location=${this.state.selLocation}&service=${this.state.selService}`
  ...
  onGetShopDetail = async (id) => {
    const { navigate } = this.props.navigation;
    try {
      let response = await fetch(`http://9330ffa5.ngrok.io/DivingBackend/public/api/shops/${id}`);
